declare module 'sphere-knn' {
	...
	type WithLongitude = { lon: number } | { lng: number } | { long: number } | { longitude: number };
	...
	type Lookup = (latitude: number, longitude: number, limit?: number, maxDistance?: number) => Geolocation[];
